...
# Use a writable temp directory so this works on read-only filesystems
# (Streamlit Cloud, Docker, etc.) as well as locally.
_WRITABLE_DIR = Path(tempfile.gettempdir()) / "quotex-api-runtime"
_WRITABLE_DIR.mkdir(parents=True, exist_ok=True)

base_dir = _WRITABLE_DIR
config_path = _WRITABLE_DIR / "settings" / "config.ini"
